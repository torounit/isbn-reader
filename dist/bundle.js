...
BarcodeReader = {
  ...
  SetLocalizationCallback: function SetLocalizationCallback(callBack) {
  ...
  SwitchLocalizationFeedback: function SwitchLocalizationFeedback(bool) {
    BarcodeReader.Config.LocalizationFeedback = bool;
  },

  // Switches for changing the Multiple property.
  DecodeSingleBarcode: function DecodeSingleBarcode() {
    BarcodeReader.Config.Multiple = false;
  },
  DecodeMultiple: function DecodeMultiple() {
  ...
  SetDecodeFormats: function SetDecodeFormats(formats) {
  ...
  SkipFormats: function SkipFormats(formats) {
    for (var i = 0; i < formats.length; i++) {
  ...
  AddFormats: function AddFormats(formats) {
    for (var i = 0; i < formats.length; i++) {
      if (BarcodeReader.SupportedFormats.indexOf(formats[i]) !== -1) {
  ...
  BarcodeReaderImageCallback: function BarcodeReaderImageCallback(e) {
  ...
  BarcodeReaderStreamCallback: function BarcodeReaderStreamCallback(e) {
    if (e.data.success === "localization") {
      if (BarcodeReader.Config.LocalizationFeedback) {
        BarcodeReader.LocalizationCallback(e.data.result);
      }
  ...
  DecodeStream: function DecodeStream(stream) {
  ...
  StopStreamDecode: function StopStreamDecode() {
    BarcodeReader.DecodeStreamActive = false;
    BarcodeReader.Decoded = [];
  },

  BarcodeReaderDecodeImage: function BarcodeReaderDecodeImage(image, orientation, sceneCaptureType) {
  ...
  FixCanvas: function FixCanvas(canvas) {
    var ctx = canvas.getContext('2d');
    ...
    ctx.drawImage = function (img, sx, sy, sw, sh, dx, dy, dw, dh) {
      ...
      if (arguments.length === 9) drawImage.call(ctx, img, sx, sy, sw, sh, dx, dy, dw, dh / vertSquashRatio);else if (typeof sw !== 'undefined') drawImage.call(ctx, img, sx, sy, sw, sh / vertSquashRatio);else drawImage.call(ctx, img, sx, sy);
    };
    return canvas;
  }
};